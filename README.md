...
个人知识库系统。将 Markdown 文件通过 LLM 自动转化为结构化 Wiki，并以知识图谱方式可视化。支持深色 / 浅色主题，智能问答采用语义分块检索 + LLM Reranking。
<img width="1209" height="935" alt="Screenshot 2026-05-04 at 20 34 21" src="https://github.com/user-attachments/assets/8adf03b5-6262-4250-a77b-b0925ac20e8d" />
...
| `QUERY_TOP_K` | `10` | 块数 | 检索后传给 LLM 的最多语义块数 |
| `QUERY_CHUNK_SIZE` | `1500` | 字符 | 语义分块大小；按 heading 切分后滑动窗口，块间 150 字符重叠 |
| `QUERY_RERANK` | `true` | — | 是否启用 LLM Reranking；开启时扩大候选池到 3×TOP_K 再二次排序 |
| `QUERY_MIN_SCORE` | `0` | 无量纲分 | 最低相关性分数（大型知识库可设 5–20） |
...
支持深色 / 浅色两套主题，通过左侧菜单底部按钮手动切换，也会跟随系统偏好自动设置。主题选择持久化到 `localStorage`，页面加载前注入以避免白屏闪烁。
...
输入自然语言问题，系统通过三阶段检索流程返回答案：

1. **语义分块**：Wiki 页面按 Markdown heading 切分为 ~1500 字符的语义块，段内过长时做滑动窗口（块间 150 字符重叠）
2. **关键词初筛**：关键词频次 + bigram 评分，从所有块中选出 3×TOP_K 候选
3. **LLM Reranking**：将候选块摘要发给 LLM（极低 token 消耗），语义选出最相关的 TOP_K 块送入上下文

支持时态查询（"今天摄入的文章"、"最近的内容"等），系统会自动注入最新摄入页面并在 system prompt 中提供当日日期。

回答中的 `[[wiki链接]]` 可点击，在右侧面板直接查看对应页面内容。每次打开页面默认开启新对话，历史对话可在左侧列表切换。
